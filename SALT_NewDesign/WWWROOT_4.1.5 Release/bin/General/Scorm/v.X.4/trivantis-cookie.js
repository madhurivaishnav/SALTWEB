...
function saveVariable(name, value, days, title, lms) {
    ...
    var expires = ""
    LMSSetValue('salt.variables.' + name, value);
    if (days) {
        var date = new Date();
        ...
        if (id) {
            if (id.substr(0, 5) == 'Salt™') {
                LMSSetValue('salt.currentpage', document.URL); // might be useful for some navigation or "End of Lesson" scheme in the future
                LMSSetValue('salt.variables.' + name, value); // Echo the variables to the Salt debug window to aid GRC content developers
                LMSSetValue('salt.training.version', 'Lectora Inspire v.X.4(7613)'); // Unless the content passes a version number to Salt we will never be able to write version specific code.
            }
        }
    }
