...
TMPr.loadTest = function(fn, tn, pn) {  // not called in later content
    var id = LMSGetValue('cmi.core.student_id');
    if (id) {
        if (id.substr(0, 5) == 'Salt™') { //LMS is Salt™
            LMSSetValue('salt.training.quizURL', tn + '.html'); // might give us the quiz url in older lessons?
            LMSSetValue('salt.training.version', 'Lectora Inspire v.X.4(7613)');
        }
    }
...
TTPr.loadTestFile = function(nl) {
    ...
    if (id) {
        if (id.substr(0, 5) == 'Salt™') { //LMS is Salt™
            //get the basics from Salt
            this.iPassGrade = new String(LMSGetValue("cmi.core.quizpassmark"));
            this.strStudentName = new String(LMSGetValue("cmi.core.student_name"));
            this.iNumberRandom = new String(LMSGetValue("cmi.core.numrandom"));
            this.bLesson = new Boolean(LMSGetValue("salt.lessonorquiz") == "lesson");
        }
    }
