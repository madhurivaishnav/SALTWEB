...
{  // not called in later content
  ...
  if (id)
  {
		if (id.substr(0,5)=='Salt™') { //LMS is Salt™
			...
			LMSSetValue( 'salt.training.version','Lectora Professional Publishing Suite v.9a(5847)');
		}
  }
...
TTPr.loadTestFile = function( nl )
{
  ...
  //MySetValue( "cmi.core.numrandom", this.iNumberRandom );
  this.iPassGrade = getNVInt( nl, 'passinggrade', this.iPassGrade );
   // MySetValue( "cmi.core.quizpassmark", this.iPassGrade );
  var id = LMSGetValue('cmi.core.student_id');
  if (id)
  {
		if (id.substr(0,5)=='Salt™') { //LMS is Salt™
