...
function isQuiz() {
    var qless = new String(LMSGetValue("salt.lessonorquiz"));
    qless = qless.toLowerCase();
    if (qless == "quiz")
        return true;
    else
        return false;
...
function putSCORMInteractions(id,obj,tim,typ,crsp,wgt,srsp,res,lat,txt,txt2) {
  ...
  if(txt2)  LMSSetValue( root + '.question.text', txt2 )
}
