...
namespace Bdw.Framework.Deployment.DatabaseConfiguration
{
    ...
    /// FrmSelectDatabase
    /// </summary>
	public class FrmSelectDatabase : Bdw.Framework.Deployment.DatabaseConfiguration.FrmBase
	{
		...
		private static FrmSelectDatabase form;

        /// <summary>
        /// FrmSelectDatabase
        /// </summary>
		public static FrmSelectDatabase Form
		{
			get
			{
		...
		private void InitializeComponent()
		{
			...
			this.lblGuide.Text = "You are required to have dbo privileges to create or upgrade database schema and " +
				"set web site SQL user permission.";
			// 
			// btnBack
			// 
		...
		private void FrmSelectDatabase_Load(object sender, System.EventArgs e)
		{
            ...
            catch (Exception ex)
            {
                ...
                //MessageBox.Show(" Error 00: Exception in ServerList.Get :" + ex.Message + Environment.NewLine + ex.StackTrace);
            }
            try
            {            
                this.cboServer.Text = ApplicationState.ServerName;
            }
            catch (Exception ex)
            {
                MessageBox.Show(" Error 0A: ApplicationState.ServerName exception :" + ex.Message + Environment.NewLine + ex.StackTrace);
            }
            try
            {   
			    this.cboDatabase.Text = ApplicationState.DatabaseName;
            }
            catch (Exception ex)
            {
                MessageBox.Show(" Error 0B: ApplicationState.DatabaseName exception :" + ex.Message + Environment.NewLine + ex.StackTrace);
            }
