...
namespace Bdw.Framework.Deployment.DatabaseConfiguration
{
    ...
    /// FrmSelectWebSQLUser
    /// </summary>
	public class FrmSelectRptWebSQLUser : Bdw.Framework.Deployment.DatabaseConfiguration.FrmBase
	{
		...
		private static FrmSelectRptWebSQLUser form;

        /// <summary>
        /// FrmSelectWebSQLUser
        /// </summary>
		public static FrmSelectRptWebSQLUser Form
		{
			get
			{
				...
				if (form==null)
				{
					form = new FrmSelectRptWebSQLUser();
				}
				return form;
			}
		}
		...
		private void InitializeComponent()
		{
			...
			this.lblTitle.Size = new System.Drawing.Size(215, 19);
			this.lblTitle.Text = "Select Reporting Web SQL Login";
			// 
			// lblGuide
			// 
			this.lblGuide.Name = "lblGuide";
			this.lblGuide.Size = new System.Drawing.Size(410, 16);
			this.lblGuide.Text = "This SQL Server Login will be used by the web site to access reporting database.";
			// 
			// btnBack
			// 
			...
			this.Name = "FrmSelectRptWebSQLUser";
			this.Text = " Database Configuration";
			this.Load += new System.EventHandler(this.FrmSelectRptWebSQLUser_Load);
		...
		private void btnNext_Click(object sender, System.EventArgs e)
		{
			if (SaveState())
			{
				...
				{
					FrmReportAdmin.Form.ShowDialog();
				}
				else
				{
		...
		private void FrmSelectRptWebSQLUser_Load(object sender, System.EventArgs e)
		{
			string webSQLLogin;
			webSQLLogin =  ConfigurationSettings.AppSettings["RptWebSQLLogin"].ToLower();
			/*  WEBSQLLOGIN 
			...
			*/
			...
			{
				this.txtUsername.Text = ApplicationState.RptWebUsername;
				this.txtPassword.Text = ApplicationState.RptWebPassword;
			}
		...
		private bool SaveState()
		{
			...
			{
				connectionString = SqlTool.GetConnectionString(ApplicationState.RptServerName,"Master",ApplicationState.RptDboUserType,ApplicationState.RptDboUsername,ApplicationState.RptDboPassword);
				try
				{
					...
					if (dt.Rows.Count==0)
					{
						MessageBox.Show("The selected NT login doesn't exist in the SQL server " + ApplicationState.RptServerName,"Invalid Login", MessageBoxButtons.OK);
						isValid = false;
					}
					else
					{
			...
			{
				connectionString = SqlTool.GetConnectionString(ApplicationState.RptServerName,"Master",userType,userName,password);
			...
			if (isValid)
			{
				ApplicationState.RptWebUserType = userType;
				ApplicationState.RptWebNTUsername  = ntUserName;
				ApplicationState.RptWebUsername  = userName;
				ApplicationState.RptWebPassword = password;
				ApplicationState.RptWebGrantPermission = grantPermission;
			}
