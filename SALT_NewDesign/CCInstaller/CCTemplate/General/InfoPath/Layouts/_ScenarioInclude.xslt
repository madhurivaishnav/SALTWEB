...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="TextBox|Graphic|FreeTextQA|MultiChoiceQA|TextGraphic|EndLesson|ExtraInfo|NumberOfQuestions|SubmitAnswers|TOC|Question|ShowAllPlayers">
	...
	<xsl:template name="PageTitle">
		<div class="PageTitle">
			<xsl:choose>
				<xsl:when test="PageElement/Question"><xsl:value-of select="$QuestionText" />&#160;<xsl:value-of select="$QuestionIndex" /> of <xsl:value-of select="$QuestionCount" /></xsl:when>
				<xsl:otherwise>
	...
	<xsl:template name="ButtonHome">
		<div class="Navigation_Button_General_IconVersion">
			<a href="javascript:__doPostBack('Home_Click','')">
				<img src="/General/Images/InfoPath/Home_Big.jpg" border="0" alt="{$HomeText}" /></a>
		</div>
	</xsl:template>
	<xsl:template name="ButtonQuizInstructions">
		<div class="Navigation_Button_General_IconVersion">
			<a href="javascript:__doPostBack('Home_Click','')">
				<img src="/General/Images/InfoPath/Instructions_Big.jpg" border="0" alt="{$InstructionsText}" /></a>
		</div>
	</xsl:template>
	...
	<xsl:template name="PageCount">
		<span class="PageFont"><xsl:value-of select="$PageText"/>&#160;<xsl:value-of select="$PageIndex+1"/>&#160;<xsl:value-of select="$OfText"/>&#160;<xsl:value-of select="$PageCount"/></span>
	</xsl:template>
